        ...
        public void AutoId_Test()
        {
            using (var db = new LiteDatabase(DB.Path()))
            {
                ...
                try
                {
                    ...
                    Assert.True(false, "Must throw and exception because EntityString class has a defined Id but has no value and no auto_id funtion - issue #43");
                }
                catch (LiteException)
                {
                ...
                // test for oid
                Assert.NotEqual(coid_1, new EntityOid());
